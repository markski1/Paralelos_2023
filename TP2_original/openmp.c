...
int main(int argc, char * argv[]) {
	if (argc > 4 || argc < 3 || atoi(argv[1]) <= 0 || atoi(argv[2]) <= 0 || atoi(argv[3]) <= 0) {
		printf("Proveer N, blocksize (opcionalmente tambien NUM_THREADS) en args.\n");
		return 1;
	}
	...
	// Si se especificara un numero de hilos.
	// Caso contrario, el sistema lo decide. Por ejemplo con OMP_NUM_THREADS si se usa batch del cluster.
	if (argc == 4) {
